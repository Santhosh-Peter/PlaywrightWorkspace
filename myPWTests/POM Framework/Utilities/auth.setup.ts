...
import test, {expect, test as setupTest} from '@playwright/test';
import path from 'path';


const authenticationFile = path.join(__dirname, '../.auth/user.json');


test('Amazon authentication setup', async({page}) => {
    ...
    await profile.hover();
    // await page.evaluate(() => {
    //     document.querySelector('#nav-al-signin')?.click()
    //     });
